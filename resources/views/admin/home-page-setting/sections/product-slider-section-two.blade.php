@php
    ...
    $sliderSectionTwo = isset($sliderSectionTwo) ? json_decode($sliderSectionTwo->value, true) : [
        'category' => null,
        'sub_category' => null,
        'child_category' => null,
    ];
    ...
    $isEmpty = !(
        !empty($sliderSectionTwo['category']) ||
        !empty($sliderSectionTwo['sub_category']) ||
        !empty($sliderSectionTwo['child_category'])
    );
@endphp

<div class="tab-pane fade" id="list-slider-two" role="tabpanel" aria-labelledby="list-slider-section-two">
                        ...
                        <div class="form-group">
                            ...
                            <select name="cat_1" class="form-control main-category">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ $category->id == $sliderSectionTwo['category'] ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="sub_cat_1" class="form-control sub-category">
                                ...
                                @foreach ($subCategories as $subCategory)
                                    <option value="{{ $subCategory->id }}" {{ $subCategory->id == $sliderSectionTwo['sub_category'] ? 'selected' : '' }}>
                                        {{ $subCategory->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="child_cat_1" class="form-control child-category">
                                ...
                                @foreach ($childCategories as $childCategory)
                                    <option value="{{ $childCategory->id }}" {{ $childCategory->id == $sliderSectionTwo['child_category'] ? 'selected' : '' }}>
                                        {{ $childCategory->name }}
                                    </option>
                                @endforeach
                            </select>
