<div class="tab-pane fade show" id="list-stripe" role="tabpanel" aria-labelledby="list-stripe-list">
    <div class="card border">
        <div class="card-body">
            <form action="{{ route('admin.stripe-setting.update', $stripeSetting->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select name="currency_name" id="currency_name" class="form-control select2">
                        ...
                        @foreach (config('settings.currency_list') as $key => $currency)
                            <option
                                {{ $currency === $stripeSetting->currency_name ? 'selected' : '' }} value="{{ $currency }}">{{ $key }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label>Locale</label>
                    <select name="locale_name" id="" class="form-control select2">
                        <option value="">Select</option>
                        @foreach (config('settings.locale') as $key => $locale)
                            <option value="{{ $key }}"
                                {{ $key === $stripeSetting->locale_name ? 'selected' : '' }}>
                                {{ $locale }} - {{ $key }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label>Currency rate (Per <span id="get-currency-name">{{ $stripeSetting->currency_name }}</span>)</label>
    ...
    <script>
        $(document).ready(function () {
            $('#currency_name').on('change', function () {
                ...
                $('#get-currency-name').text(selectedCurrency);
            });
        });
    </script>
@endpush
